...
function get_services_from_user_input_with_geolocation(string $user_input, float $latitude, float $longitude): array
{
  // build prompt db
  $services = ['Columns' => [], 'Rows' => []];
  foreach (get_services()[0] as $key => $value) {
  ...
  // add distances from user to prompt db
  $services['Columns'][] = 'DistanceFromUser';
  foreach ($services['Rows'] as &$row) {
    $service_coordinates = get_service_coordinates(service_id: (int) $row[0]);
    $service_latitude = $service_coordinates['latitude'];
    $service_longitude = $service_coordinates['longitude'];
    if (!$service_latitude or !$service_longitude) {
      $row[] = 'Unknown';
      continue;
    }
    $row[] = haversine(lat1: $latitude, lon1: $longitude, lat2: $service_latitude, lon2: $service_longitude);
  }
  unset($row);
  $services = json_encode(value: $services);
